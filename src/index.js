...
async function sanitizeFile (filePath, outDir, jsonObj, linksOrder = {}, write = false) {
    ...
    if (ext === '.html') {
        try {
            ...
            write && await writeFile(newFilePath, html);

            if (rootDirName === dirname) {
                rootDirPath = `${inputDir}/${basename}`;
                await traverseDirectory(filePath.replace('.html', ''), jsonObj, links, write);
            } else {
                if (hasLinks) {
                    jsonObj[basename] = {
                        subPages: {},
                    ...
                    await traverseDirectory(filePath.replace('.html', ''), jsonObj[basename].subPages, links, write);
                } else {
                    if (!jsonObj[basename]) {
                        jsonObj[basename] = {
                            title: basename,
...
async function processFiles (files, dir, outDir, jsonObj, linkTitles, write) {
    return await Promise.all(files.map(async file => {
        const filePath = path.resolve(dir, file);
        return await sanitizeFile(filePath, outDir, jsonObj, linkTitles, write);
    }));
}

async function traverseDirectory (dir, jsonObj = {}, linkTitles = {}, write) {
    try {
        const files = await readdir(dir);
        const newDir = `${outDirPath}${dir.replace(`${inputDir}`, '')}`;
        await mkdir(newDir);
        await processFiles(files, dir, newDir, jsonObj, linkTitles, write);
    }  catch (e) {
        throw new Error(e);
    }
}
...
traverseDirectory(inputDir, json, {}, false).then(() => {
    removeDir(outDirPath);
    traverseDirectory(inputDir, json, {}, true).then(() => {
